...
void solve() {
    ...
    while (t--) {
        ...
        for(int i=0;i<n;i++){
            for(int j=i+1;j<n;j++){
                auto it=lower_bound(all(a),a[i]+a[j])-a.begin()-1;
                while(it>=0 && (i==it || j==it))it--;
                if(it>=0)ans=max(ans,a[it]%(a[i]+a[j]));
                it=lower_bound(all(a),a[i]+a[j])-a.begin();
                while(it<n && (i==it || j==it))it++;
                if(it<n)ans=max(ans,a[it]%(a[i]+a[j]));
            }
        }
        cout<<ans<<endl;
    }
