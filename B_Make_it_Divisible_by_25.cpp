...
void print(vs a) {for (auto i : a) {cout << i << " ";}cout << endl;}
...
ll check(s a,s b){
    ll c=0,c1=0;
    for(int i=a.length()-1;i>=0 && !b.empty();i--){
        if(a[i]==b.back()){
            b.pop_back();
            c1++;
        }
...
void solve() {
    ...
    while (t--) {
        ...
        if(a.length()<2){
            ...
            cout<<min({check(a,"00"),check(a,"25"),check(a,"50"),check(a,"75")})<<endl;
        }
