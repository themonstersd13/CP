...
void solve() {
    ...
    while (t--) {
        ...
        for(auto i:a)dp[i]=0;
        for(auto i:b)dp[i]=0;
        for(auto i:a)dp1[i]=0;
        for(auto i:b)dp1[i]=0;
        while(i<n){
            ch[a[i]]++;
            while(j<i && ch.size()>1){
                ch[a[j]]--;
                if(ch[a[j]]==0)ch.erase(a[j]);
                j++;
            }
            if(ch.size()==1)dp[a[i]]=max(i-j+1,dp[a[i]]);
            i++;
        }
        i=0;
        j=0;
        ...
        while(i<n){
            ...
            while(j<i && ch.size()>1){
                ch[b[j]]--;
                if(ch[b[j]]==0)ch.erase(b[j]);
                j++;
            }
            if(ch.size()==1)dp1[b[i]]=max(i-j+1,dp1[b[i]]);
            i++;
        }
        ll ans=0;
        si st(all(a));
